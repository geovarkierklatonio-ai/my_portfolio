...
function ProjectCard({
  ...
  const [showIframeInModal, setShowIframeInModal] = useState(false)
  const isJdlProject = title === 'JDL Security Systems'
  const isDmtsProject = title === 'LGU Matanao DMTS'
  const isCapstoneProject = title === 'Geo Attendance System'
  const jdlDemoUrl = 'https://geovarkierklatonio-ai.github.io/jdldavao/'
  const dmtsUrl = 'https://lgumatanao-dmts.com/'
  const capstonePdfUrl = '/capstoneproject.pdf'
  ...
  return (
    <>
          ...
          <div className="relative h-52 overflow-hidden bg-slate-900">
            ...
            <div className="absolute inset-x-0 bottom-0 flex items-end justify-between p-5">
              ...
              <div className="flex items-center gap-2">
                <span className="rounded-full border border-white/20 bg-slate-950/40 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white/90 backdrop-blur-sm">
                  {year}
                </span>
                {featured ? (
                  <span className="rounded-full bg-emerald-400/30 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-50 backdrop-blur-sm">
                    Featured
                  </span>
                ) : null}
              </div>
        ...
        <div className="flex flex-1 flex-col p-6">
          ...
          <div className="mt-6 flex items-center gap-3 pt-4">
            <button
              type="button"
              onClick={() => {
                if (isJdlProject || isDmtsProject || isCapstoneProject) {
                  setShowIframeInModal(true)
                } else {
                  setIsModalOpen(true)
                }
              }}
      ...
      <AnimatePresence>
        {showIframeInModal && (isJdlProject || isDmtsProject || isCapstoneProject) ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.96, y: 16 }}
              transition={{ duration: 0.25, ease: 'easeOut' }}
              className="relative h-[90vh] w-full max-w-6xl overflow-hidden rounded-[30px] border border-white/10 bg-slate-900 shadow-[0_30px_90px_rgba(15,23,42,0.8)]"
            >
              <button
                type="button"
                onClick={() => setShowIframeInModal(false)}
                className="absolute right-4 top-4 z-20 inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-slate-950/60 text-slate-200 transition hover:border-cyan-400/60 hover:text-cyan-200"
              ...
              <div className="flex h-full flex-col bg-slate-900">
                <div className="flex items-center justify-between border-b border-white/10 bg-slate-800/50 px-6 py-4 backdrop-blur-sm">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-cyan-300">{isCapstoneProject ? 'Project PDF' : 'Live Preview'}</p>
                ...
                <div className="flex items-center justify-between border-t border-white/10 bg-slate-800/50 px-6 py-4 backdrop-blur-sm">
                  <p className="text-xs text-slate-400">{isCapstoneProject ? 'Preview of /capstoneproject.pdf' : `Preview of ${isJdlProject ? jdlDemoUrl : dmtsUrl}`}</p>
                  <button
                    type="button"
                    onClick={() => window.open(isCapstoneProject ? link : isJdlProject ? jdlDemoUrl : dmtsUrl, '_blank', 'noopener,noreferrer')}
                  ...
                  >
                    {isCapstoneProject ? 'Open live project' : 'View full page'} <ExternalLink size={15} />
                  </button>
                </div>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {isFullViewOpen && !isJdlProject ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] bg-slate-950/90 p-0 backdrop-blur-md"
          >
            <motion.div
              ...
              exit={{ opacity: 0, y: 18 }}
              transition={{ duration: 0.25, ease: 'easeOut' }}
              className="mx-auto flex h-full w-full flex-col overflow-hidden bg-slate-900"
            >
              <>
